 ...
 FETCH FIRST 1 ROWS ONLY) AS DTENCAMINHA_ANTERIOR,
   ...
   WHERE 
    ...
    AND OS.SGORGAOSETOR LIKE '%INFRA%'
        AND (
            (TU.DTRECEBTO IS NULL AND TU.DTENCAMINHA >= TO_DATE('2025-04-21 00:00:00', 'YYYY-MM-DD HH24:MI:SS'))
            OR (TU.DTRECEBTO >= TO_DATE('2025-04-21 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
                AND (SELECT TU_ANT.DTENCAMINHA
                     FROM ECPATRAMITACAO TU_ANT
                     WHERE TU_ANT.NUPROCESSO = TU.NUPROCESSO
                       AND TU_ANT.NUANO = TU.NUANO
                       AND TU_ANT.NUTRAMITE = TU.NUTRAMITE - 1
                     FETCH FIRST 1 ROWS ONLY) >= TO_DATE('2025-04-21 00:00:00', 'YYYY-MM-DD HH24:MI:SS'))
        )
)
SELECT 
    NMPROCESSO,
